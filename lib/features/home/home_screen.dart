...
const double _kHomeMapZoom =
    13.5; // slightly wider than discover (14) for overview
            ...
            Positioned(
              ...
              child: isTrainerView
                  ...
                  : AppFab(
                      ...
                      label: 'Book Session',
                      tooltip:
                          'Find and book a session with a certified trainer',
...
class _NearbyMapCardState extends ConsumerState<_NearbyMapCard> {
  ...
  void _onMapReady(AppMapController ctrl) {
    _mapController = ctrl;
    _pinTrainers(ref.read(allTrainersProvider).valueOrNull ?? []);
  }

  Future<void> _pinTrainers(List<TrainerProfile> trainers) async {
  ...
  List<Widget> _buildMapChildren(
      AsyncValue<List<TrainerProfile>> trainersAsync, AppLatLng centre) {
    ...
    // Loading overlay while trainers are fetching.
    if (trainersAsync.isLoading) {
      children.add(
        Positioned.fill(
          child: Container(
  ...
  Widget build(BuildContext context) {
    ...
    // Re-pin when trainers finish loading.
    ref.listen(allTrainersProvider, (_, next) {
      _pinTrainers(next.valueOrNull ?? []);
    });

    final trainers = trainersAsync.valueOrNull ?? [];
    final countLabel = trainersAsync.isLoading
        ? 'Finding trainers near you...'
        : trainers.isEmpty
            ? 'No trainers found nearby'
            : '${trainers.length} trainer${trainers.length == 1 ? "" : "s"} near you';

    final centre =
        locationAsync.value ?? const AppLatLng(_kFallbackLat, _kFallbackLng);
          ...
          children: [
            ...
            Padding(
              padding: const EdgeInsets.fromLTRB(
                  _kMapCardPadding, _kMapCardPadding, _kMapCardPadding, 14),
              child: Row(
            ...
            ClipRRect(
              ...
              child: SizedBox(
                height: 240,
                child: Stack(
                  children: _buildMapChildren(trainersAsync, centre),
                ),
              ),
            ),
          ],
...
class _BookingCalendarCardState extends State<_BookingCalendarCard> {
  ...
  void _onDayTap(int day) {
    final dayBookings = widget.bookings.where((b) {
    ...
    if (dayBookings.isNotEmpty) {
      showDialog(
        ...
        builder: (ctx) => AlertDialog(
          title: Text('Sessions on ${_mn[_month.month - 1]} $day'),
          content: SizedBox(
            ...
            child: ListView(
              ...
              children: dayBookings.map((b) {
                final time =
                    b.slotStartTime!.toLocal().toString().substring(11, 16);
                return ListTile(
                  title: Text(b.trainerName),
                  subtitle: Text('Session at $time'),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          _CalLegend(
              color: AppColors.primary, dot: true, label: 'Session booked'),
          const Spacer(),
    ...
    return Row(children: [
      ...
      Expanded(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('${target.toInt()} $unit target', style: AppTypography.caption),
        ]),
      ),
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Wrap(spacing: 8, runSpacing: 6, children: [
        _InfoPill(
          ...
          label:
              '${_dy[start.weekday - 1]}, ${_mo[start.month - 1]} ${start.day}',
        ),
...
class _GoalsCardState extends State<_GoalsCard>
  ...
  int get _weeklyWorkouts {
    ...
    return widget.logs
        .where(
            (l) => l.type == WellnessType.workout && l.timestamp.isAfter(cut))
        .length;
  }
        ...
        AnimatedBuilder(
          ...
          builder: (_, __) => Column(children: [
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text('${startKg.toInt()}kg', style: AppTypography.caption),
        ...
        Row(children: [
          ...
          ...List.generate(
            ...
            (i) => Container(
              ...
              decoration: BoxDecoration(
                color: i < workouts ? AppColors.primary : AppColors.surfaceMid,
                shape: BoxShape.circle,
                border: Border.all(color: AppColors.border),
              ),
            ),
...
class _TrainerStatsCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final upcoming = bookings
        .where((b) =>
            b.slotStartTime != null && b.slotStartTime!.isAfter(DateTime.now()))
        .length;
