...
const _kAdminNav = [
  _AdminNavItem(label: 'Overview',  icon: Icons.dashboard_outlined,     route: '/admin'),
  _AdminNavItem(label: 'Content',   icon: Icons.edit_outlined,           route: '/admin/content'),
  _AdminNavItem(label: 'Brand',     icon: Icons.palette_outlined,        route: '/admin/brand'),
  _AdminNavItem(label: 'Features',  icon: Icons.toggle_on_outlined,      route: '/admin/features'),
  _AdminNavItem(label: 'Preview',   icon: Icons.visibility_outlined,     route: '/admin/preview'),
];

class QAdminSidebar extends StatelessWidget {
  final bool expanded;
  final VoidCallback onToggle;
  const QAdminSidebar({super.key, required this.expanded, required this.onToggle});
          ...
          child: ListView(
            ...
            children: _kAdminNav.map((item) {
              final active = current.startsWith(item.route) &&
                  (item.route == '/admin' ? current == '/admin' : true);
              return _SidebarItem(item: item, active: active, expanded: expanded);
            }).toList(),
          ),
        ),
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(Icons.arrow_back_rounded, size: 14, color: AppColors.secondary),
                if (expanded) ...[
                  SizedBox(width: 6),
                  Text('Live site', style: AppTypography.caption.copyWith(
...
class _SidebarItem extends StatefulWidget {
  ...
  final bool active;
  final bool expanded;
  const _SidebarItem({required this.item, required this.active, required this.expanded});
...
class _SidebarItemState extends State<_SidebarItem> {
  ...
  Widget build(BuildContext context) {
    final bg = widget.active
        ...
        : _hovered ? AppColors.primary.withValues(alpha: 0.06)
        : Colors.transparent;
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          child: widget.expanded
              ...
              : Center(child: Icon(widget.item.icon, size: 20, color: color)),
        ),
      ),
    );
